...
- Removed `source` parameter from `update_emote`, `safe_update_emote`, and `import_emotes` — CyTube no longer uses the `source` field; emotes are now `{name, image}` only.
...
### Added
- `ChatMessageEvent.shadow` (`bool`, default `False`): set to `True` when CyTube delivers
  a message from a shadow-muted user (`meta.shadow` in the raw `chatMsg` payload).
  Consumers should silently ignore messages where `shadow=True`.
...
### Fixed
- **CI**: Fixed `black` formatting inconsistencies in `client.py` following linting fixes.

## [0.11.3] - 2025-12-31

### Fixed
- **Linting**: Resolved unused variable `nats_client` errors (Ruff F841) in `client.py`.
